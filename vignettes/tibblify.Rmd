...
As seen before the specification for a collection is done with `spec_df()`. The
...
```{r}
spec <- spec_guess(gh_repos_small)
spec
```
...
You can specify the value to use with the `default` argument

```{r}
spec <- spec_df(
  x = tib_int("x"),
  y = tib_chr("y", required = FALSE, default = "missing")
)
...
To rectangle a single object you have two options: `spec_object()` which produces
a list or `spec_row()` which produces a tibble with one row.
