...
tibblify_impl <- function(recordlist, col_specs, keep_spec, names_to = NULL, parent_paths = character()) {
  ...
  result_list <- purrr::map(
    collectors_non_skip,
    function(collector) {
      valueslist <- extract_index(recordlist, collector$path, collector$.default, parent_paths)
...
extract_index <- function(recordlist, index, default, parent_paths) {
  tryCatch(
    expr = {
      if (is_zap(default)) {
    ...
    error = function(x) {
      abort(paste0("empty or absent element at path ", paste0(c(parent_paths, index), collapse = "/")))
    }
  )
}
