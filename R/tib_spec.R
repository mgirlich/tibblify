...
inform_unspecified <- function(spec) {
  unspecified_paths <- get_unspecfied_paths(spec)


  inform("The following elements are unspecified:")
  lines <- format_unspecified_tree(unspecified_paths)
  paste0(lines, collapse = "\n") %>% cat()
}

format_unspecified_tree <- function(path_list) {
  box_chars <- cli_box_chars()
  ...
  for (i in seq_along(path_list)) {
    ...
    if (is.character(elt)) {
      ...
      lines <- c(lines, paste0(box, elt))
    } else {
      sub_lines <- format_unspecified_tree(elt)

      if (i < n) {
        sub_lines <- paste0(box_chars$v, " ", sub_lines)
      } else {
