...
test_that("scalar POSIXlt is converted to POSIXct", {
  ...
  expect_equal(
    tspec_guess_df(tibble(x = x_posixlt)),
...
test_that("can guess vector columns", {
  ...
  x_rat <- new_rational(1, 2)
  expect_equal(
    tspec_guess_df(tibble(x = list(x_rat))),
...
test_that("vector POSIXlt is converted to POSIXct", {
  x_posixlt <- as.POSIXlt(vctrs::new_date(0))
  expect_equal(
    tspec_guess_df(tibble(x = list(x_posixlt))),
...
test_that("can guess vector NA columns", {
  ...
  expect_equal(
    tspec_guess_df(tibble(x = list(c(NA, NA), NA))),
...
test_that("respect empty_list_unspecified for vector columns", {
  ...
  expect_equal(
    tspec_guess_df(x, empty_list_unspecified = FALSE),
    tspec_df(int_vec = tib_variant("int_vec"))
  )

  expect_equal(
    tspec_guess_df(x, empty_list_unspecified = TRUE),
    tspec_df(
...
test_that("can guess list of NULL columns", {
  expect_equal(
    tspec_guess_df(tibble(x = list(NULL, NULL))),
...
test_that("can guess mixed columns", {
  expect_equal(
    tspec_guess_df(tibble(x = list(1, "a"))),
...
test_that("can guess non-vector objects", {
  ...
  expect_equal(
    tspec_guess_df(tibble(x = list(model, 1))),
    tspec_df(x = tib_variant("x"))
  )
})
...
test_that("can guess tibble columns", {
  # scalar
  expect_equal(
    tspec_guess_df(tibble(df = tibble(int = 1L, chr = "a"))),
    tspec_df(
      df = tib_row(
        "df",
        int = tib_int("int"),
  ...
  # vector
  expect_equal(
    tspec_guess_df(tibble(df = tibble(int_vec = list(1L)))),
    tspec_df(
  ...
  # mixed
  expect_equal(
    tspec_guess_df(tibble(df = tibble(x = list(1L, "a")))),
    tspec_df(
  ...
  # tibble -> recursion
  expect_equal(
    tspec_guess_df(tibble(df = tibble(x = tibble(y = 1L)))),
    tspec_df(
