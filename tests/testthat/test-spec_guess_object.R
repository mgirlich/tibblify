test_that("can guess scalar elements", {
  expect_equal(
    tspec_guess_object(list(x = TRUE)),
    tspec_object(x = tib_lgl("x"))
  )

  expect_equal(
    tspec_guess_object(list(x = new_datetime(1))),
  ...
  expect_equal(
    tspec_guess_object(list(x = x_rat)),
...
test_that("POSIXlt is converted to POSIXct", {
  ...
  expect_equal(
    tspec_guess_object(list(x = x_posixlt)),
...
test_that("can handle non-vector elements", {
  ...
  expect_equal(
    tspec_guess_object(list(x = model)),
...
test_that("can guess vector elements", {
  expect_equal(
    tspec_guess_object(list(x = c(TRUE, FALSE))),
    tspec_object(x = tib_lgl_vec("x"))
  )

  expect_equal(
    tspec_guess_object(list(x = c(new_datetime(1), new_datetime(2)))),
  ...
  x_rat <- new_rational(1, 2)
  expect_equal(
    tspec_guess_object(list(x = c(x_rat, x_rat))),
...
test_that("POSIXlt is converted to POSIXct for vector elements", {
  x_posixlt <- as.POSIXlt(vctrs::new_date(0))
  expect_equal(
    tspec_guess_object(list(x = c(x_posixlt, x_posixlt))),
...
test_that("can guess tib_vector for a scalar list", {
  expect_equal(
    tspec_guess_object(list(x = list(TRUE, TRUE, NULL)), simplify_list = FALSE),
    tspec_object(x = tib_variant("x"))
  )

  expect_equal(
    tspec_guess_object(list(x = list(TRUE, TRUE, NULL)), simplify_list = TRUE),
    tspec_object(x = tib_lgl_vec("x", input_form = "scalar_list"))
  )

  expect_equal(
    tspec_guess_object(list(x = list(new_datetime(1))), simplify_list = TRUE),
    tspec_object(x = tib_vector("x", new_datetime(), input_form = "scalar_list"))
  )

  # checks size
  expect_equal(
    tspec_guess_object(list(x = list(1, 1:2)), simplify_list = TRUE),
    tspec_object(x = tib_variant("x"))
  )

  expect_equal(
    tspec_guess_object(list(x = list(1, integer())), simplify_list = TRUE),
    tspec_object(x = tib_variant("x"))
  )
})
...
test_that("can guess tib_vector for input form = object", {
  expect_equal(
    tspec_guess_object(list(x = list(a = TRUE, b = TRUE)), simplify_list = TRUE),
...
test_that("can guess mixed elements", {
  expect_equal(
    tspec_guess_object(list(x = list(TRUE, "a"))),
    tspec_object(x = tib_variant("x"))
  )
})
...
test_that("can handle non-vector elements in list", {
  model <- lm(Sepal.Length ~ Sepal.Width, data = iris)
  expect_equal(
    tspec_guess_object(list(x = list(model, model))),
    tspec_object(x = tib_variant("x"))
  )
})
...
test_that("can guess df element", {
  expect_equal(
    tspec_guess_object(list(x = tibble(a = 1L))),
...
test_that("can guess tib_row", {
  expect_equal(
    tspec_guess_object(list(x = list(a = 1L, b = "a"))),
...
test_that("can guess tib_row with a scalar list", {
  expect_equal(
    tspec_guess_object(list(x = list(a = list(1L, 2L), b = "a")), simplify_list = TRUE),
    tspec_object(
      x = tib_row(
        "x",
...
test_that("respect empty_list_unspecified for list of object elements", {
  ...
  expect_equal(
    tspec_guess_object(x, empty_list_unspecified = FALSE),
    tspec_object(
      x = tib_df(
        "x",
        a = tib_variant("a"),
  ...
  expect_equal(
    tspec_guess_object(x, empty_list_unspecified = TRUE),
    tspec_object(
...
test_that("can guess tib_unspecified for an object", {
  ...
  expect_equal(
    tspec_guess_object(list(x = NULL)),
    tspec_object(x = tib_unspecified("x"))
  )

  # empty lists could be object or list of object -> unspecified
  expect_equal(
    tspec_guess_object(list(x = list()), empty_list_unspecified = FALSE),
    tspec_object(x = tib_unspecified("x"))
  )

  # NA could be any scalar value
  expect_equal(
    tspec_guess_object(list(x = NA)),
    tspec_object(x = tib_unspecified("x"))
  )

  # TODO not yet decided
  # expect_equal(
  #   tspec_guess_object(list(x = list(NULL, NULL))),
  #   tspec_object(x = tib_unspecified("x"))
  # )

  # in a row
  expect_equal(
    tspec_guess_object(list(x = list(a = NULL))),
    tspec_object(x = tib_row("x", a = tib_unspecified("a")))
  )

  expect_equal(
    tspec_guess_object(list(x = list(a = list()))),
    tspec_object(x = tib_row("x", a = tib_unspecified("a")))
  )

  expect_equal(
    tspec_guess_object(list(x = list(a = list())), empty_list_unspecified = FALSE),
    tspec_object(x = tib_row("x", a = tib_unspecified("a")))
  )
